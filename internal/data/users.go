...
import (
	...
	"greenlight.mohit.net/internal/validator"
)
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	if user.Password.hash == nil {
		panic("missing passowrd hash for user")
	}
}
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	...
	err := m.DB.QueryRowContext(ctx, q, email).Scan(&user.ID, &user.CreatedAt, &user.Name, &user.Email, &user.Password.hash, &user.Activated, &user.Version)
	if err != nil {
		switch {
...
func (m UserModel) Update(user *User) error {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	err := m.DB.QueryRowContext(ctx, q, args...).Scan(&user.Version)
	if err != nil {
		switch {
		case err.Error() == `pq: duplicate key value violates unique constraint "users_email_key"`:
			return ErrDuplicateEmail
